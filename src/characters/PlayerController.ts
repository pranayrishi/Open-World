...
export class PlayerController {
  ...
  fixedUpdate(deltaTime: number): void {
    ...
    this.checkWorldBoundary(currentPos);
  }

  private checkWorldBoundary(currentPos: THREE.Vector3): void {
    const nextChunkX = Math.floor(currentPos.x / CHUNK_SIZE);
    const nextChunkZ = Math.floor(currentPos.z / CHUNK_SIZE);
    const key = `${nextChunkX},${nextChunkZ}`;

    if (!this.chunkManager.isChunkLoaded(key)) {
      const clampedX = this.clampToLoadedChunks(
        currentPos.x,
        currentPos.z,
        "x",
      );
      const clampedZ = this.clampToLoadedChunks(
        currentPos.x,
        currentPos.z,
        "z",
      );
      ...
      if (clampedX !== currentPos.x || clampedZ !== currentPos.z) {
        this.playerBody.setTranslation(
          { x: clampedX, y: currentPos.y, z: clampedZ },
          true,
        );
        const vel = this.playerBody.linvel();
        this.playerBody.setLinvel(
          {
            x: clampedX !== currentPos.x ? 0 : vel.x,
            y: vel.y,
            z: clampedZ !== currentPos.z ? 0 : vel.z,
          },
          true,
        );
      }
  ...
  private clampToLoadedChunks(x: number, z: number, axis: "x" | "z"): number {
    const chunkCoord =
      axis === "x" ? Math.floor(x / CHUNK_SIZE) : Math.floor(z / CHUNK_SIZE);
    const otherCoord =
      axis === "x" ? Math.floor(z / CHUNK_SIZE) : Math.floor(x / CHUNK_SIZE);

    const key =
      axis === "x"
        ? `${chunkCoord},${otherCoord}`
        : `${otherCoord},${chunkCoord}`;

    if (this.chunkManager.isChunkLoaded(key)) return axis === "x" ? x : z;

    const safeChunk =
      axis === "x"
        ? Math.floor(this.lastSafePosition.x / CHUNK_SIZE)
        : Math.floor(this.lastSafePosition.z / CHUNK_SIZE);

    if (
      (axis === "x" ? x : z) >
      (axis === "x" ? this.lastSafePosition.x : this.lastSafePosition.z)
    ) {
      return (safeChunk + 1) * CHUNK_SIZE - 1;
    } else {
      return safeChunk * CHUNK_SIZE + 1;
    }
  }
